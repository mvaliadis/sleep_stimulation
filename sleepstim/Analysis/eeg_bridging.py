...
directory = '/media/administrator/Sleep_Data/Processed/'
...
for file in os.listdir(directory):
    if file.endswith('-raw.fif'):
        ...
        raw = mne.io.read_raw(os.path.join(directory, file), preload=True, verbose=False)
        raw_data[sub] = raw.pick('eeg')
...
bridged_idx, ed_matrix = ed_data['MaMe_1']

fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))
fig.suptitle("Subject MaMe Electrical Distance Matrix")
...
fig.suptitle("Subject MaMe_1 Electrical Distance Matrix Distribution")
...
mne.viz.plot_bridged_electrodes(
    raw_data['MaMe_1'].info,
    bridged_idx,
    ed_matrix,
    title="Subject MaMe Bridged Electrodes",
...
raw = raw_data['MaMe_1'].copy().pick(["FC2", "FC4", "F2", "F4"])
raw.add_channels(
    [
        mne.io.RawArray(
...
raw.plot(duration=30, scalings=dict(eeg=2e-4))

raw = raw_data['JaKu_1'].copy().pick(["FC2", "FC4", "F2", "F4"])
raw.add_channels(
    [
        mne.io.RawArray(
            raw.get_data(ch1) - raw.get_data(ch2),
...
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))
fig.suptitle("Electrical Distance Distribution for NMTI Pilot Subjects")
for ax in (ax1, ax2):
    ax.set_ylabel("Count")
    ax.set_xlabel(r"Electrical Distance ($\mu$$V^2$)")
...
bridged_idx = ed_data['AlSt_1'][0]
raw = mne.preprocessing.interpolate_bridged_electrodes(
    raw_data['AlSt_1'].copy(), bridged_idx=bridged_idx
)
...
data_orig = raw_data['AlSt_1'].get_data(picks=(idx0, idx1))
