...
def get_coords():
    channels = np.load('channels.npy')[:,0]
    ...
    while reiz.clock.now() - tz < totalruntime:
        ...
        if sum(stage_predictArrays[-nepochsthresh:]) >= nepochsthresh:
            ...
            if channel_failure[0] == 1:
                ...
                ## apply surface laplacian (no need to reref)
                #data = surface_laplacian(data = np.expand_dims(d,1), coords=coords, sf=bfr2.fs, reject=None)
